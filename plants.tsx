...
import { plants, projects, projectWorkers, type Plant } from "@/lib/rootline-data";
...
export const Route = createFileRoute("/plants")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Plant & area register — Goldman Stocks" },
      {
        property: "og:description",
        content: "Searchable register of every managed plant and area, grouped by project.",
      },
    ],
  }),
  component: Plants,
...
const filters = ["All", "Tree", "Hedge", "Lawn", "Flower bed", "Shrub"] as const;
...
function Plants() {
    ...
    return projects
      .map((project) => ({
        ...
        rows: plants.filter((p) => {
          ...
          if (!q) return true;
          return [p.id, p.species, p.common, p.client, p.site, project.name].some((v) =>
  ...
  return (
    <AppShell
      ...
      actions={
        <Button onClick={() => toast("Open the worker app to add a plant by photo")}>
    ...
    >
        ...
        {filters.map((f) => (
          <button
            ...
            className={`rounded-full border px-3 py-1.5 text-sm transition-colors ${
              kind === f ? "border-primary bg-primary text-primary-foreground" : "hover:bg-muted"
            }`}
          >
            {f}
          </button>
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      {rows.length} items · crew {crew.map((w) => w.name.split(" ")[0]).join(", ")}
                    </p>
                  </div>
                  <Link
                  ...
                  <TableBody>
                    {rows.map((p) => (
                      <TableRow key={p.id}>
                        ...
                        <TableCell className="text-sm text-muted-foreground">{p.lastCare}</TableCell>
                        <TableCell>
      ...
      <Dialog open={Boolean(openPlant)} onOpenChange={(o) => (o ? null : setOpenPlant(null))}>
          ...
          {openPlant ? (
            <>
              <DialogHeader>
                <DialogTitle>
                  {openPlant.common}{" "}
                  <span className="font-mono text-xs text-muted-foreground">{openPlant.id}</span>
                </DialogTitle>
                <DialogDescription>
