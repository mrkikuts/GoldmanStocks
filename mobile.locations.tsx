...
import { projectPlants, projectWorkers } from "@/lib/rootline-data";
...
export const Route = createFileRoute("/mobile/locations")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Every site on the crew's list, with its address, crew and plant map.",
      },
      { property: "og:title", content: "Work sites — Goldman Stocks worker app" },
      {
...
function MobileLocations() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <p className="mb-1 text-[11px] font-bold uppercase tracking-[0.12em] text-accent-foreground">Assigned sites</p>
        <h1 className="font-display text-3xl font-bold text-primary">Locations</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          {myProjects.length} sites for {worker.name.split(" ")[0]} — tap a dot on a map to see a
          plant.
        </p>
      </div>
      ...
      {myProjects.map((project) => {
        ...
        return (
          <section key={project.id} className="space-y-3 rounded-lg border bg-card p-4 shadow-card">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <h2 className="font-display text-xl font-bold leading-tight text-primary">{project.name}</h2>
